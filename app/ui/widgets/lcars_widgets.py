...
class LcarsButton(LcarsWidget):
    ...
    def applyColour(self, colour):
        super().applyColour(colour)
        # redraw the text on top of the newly tinted image
        font = Font("assets/swiss911.ttf", 19)
        text_image = font.render(self.text, False, colours.BLACK)
        self.image.blit(text_image,
                        (self.image.get_rect().width - text_image.get_rect().width - 10,
                         self.image.get_rect().height - text_image.get_rect().height - 5))
